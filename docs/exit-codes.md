...
|---|---|
| `0` | success |
| `1` | unexpected internal error — or, as in gh, a failed check (`gf pr checks`) or run (`--exit-status`) |
...
| `8` | git context error — or, as in gh, checks still pending (`gf pr checks`) |
...
pending, and so does `gf pr checks`. `error.code` always tells them apart.
...
| `CHECKS_FAILED` | 1 | `gf pr checks`, human output: at least one check failed |
| `CHECKS_PENDING` | 8 | `gf pr checks`, human output: checks are still running |
| `RUN_FAILED` | 1 | `gf run view` / `gf run watch` / `gf pipeline view` with `--exit-status`, human output: the run failed |
...
```bash
gf --agent pr view 123 | jq -r 'if .ok then .data.state else .error.code end'
```
